...
void test_request(void)
{
  ...
  pelz_request_handler(eid, &status, request_type, tmp, data_in, chan_key, &output);
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    pelz_request_handler(eid, &status, request_type, tmp, data, chan_key, &output);
    CU_ASSERT(status == REQUEST_OK);
  ...
  pelz_request_handler(eid, &status, REQ_ENC, tmp, data_in, chan_key, &output);
  CU_ASSERT(status == KEK_NOT_LOADED);
  free_charbuf(&tmp);
